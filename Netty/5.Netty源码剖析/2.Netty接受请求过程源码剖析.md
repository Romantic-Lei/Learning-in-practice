...
1）通过工具类，调用 NioServerSocketChannel 内部封装的 serverSocketChannel 的 accept 方法(154行)，这是 Nio 做法。

2）获取到一个 JDK 的 SocketChannel，然后，使用 NioSocketChannel 进行封装。最后添加到容器中

3）这样容器 buf 中就有了 NioSocketChannel，通过断点追踪，发现在 new NioSocketChannel(this, ch) 方法中，它会将当前请求设置成 SelectionKey.OP_READ 读状态并且设置成非阻塞的，这和之前自己写的Nio流程是一样的。
